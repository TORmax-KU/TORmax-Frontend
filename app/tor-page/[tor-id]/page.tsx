...
export interface TORDetailPageProps {
  params: Promise<{
    'tor-id': string;
  }> | {
    'tor-id': string;
...
export default function TORDetailPage({ params }: TORDetailPageProps) {
  const router = useRouter();
  const routeParams = useParams();

  // Unwrap parameters passed directly via props, or fallback to useParams hook
  const resolvedParams = React.use(
    params instanceof Promise ? params : Promise.resolve(params)
  );

  // Read parameter matching the [tor-id] folder naming convention
  const targetId = resolvedParams?.['tor-id'] || (routeParams?.['tor-id'] as string);
  const tor = initialTORs.find((t) => t.id === targetId);

  if (!tor) {
    return (
      <div className="max-w-4xl mx-auto p-12 text-center space-y-4">
        <h2 className="text-xl font-bold">TOR Document Not Found</h2>
        <button 
          onClick={() => router.push('/search-feed')} 
          className="text-xs text-[#5B3E96] font-bold underline"
        >
          Back to Directory
        </button>
      </div>
    ...
    <div className="max-w-5xl mx-auto px-8 py-10 w-full space-y-8">
      ...
      <header className="bg-white dark:bg-[#1C1A24] border border-slate-200 dark:border-[#2D2938] rounded-3xl p-8 space-y-4 shadow-sm">
        <div className="flex justify-between items-start gap-4">
          <div>
            <span className="text-xs font-bold text-[#5B3E96] dark:text-[#9B82C1]">
              {tor.id} • {tor.method}
            </span>
          ...
          <div className="text-right">
            <span className="text-3xl font-black text-[#5B3E96] dark:text-[#9B82C1]">
              {tor.matchScore}%
            </span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-slate-100 dark:border-[#2D2938] text-xs">
          ...
          <div>
            <span className="block text-slate-400 font-medium">Submission Window</span>
          ...
          <div>
            <span className="block text-slate-400 font-medium">Requirement Clearance</span>
            <span className="font-bold text-slate-900 dark:text-white">
              {count}/{tor.requirements.length} Passed ({percentage}%)
            </span>
          </div>
        </div>
      </header>
      ...
      <section className="bg-white dark:bg-[#1C1A24] border border-slate-200 dark:border-[#2D2938] rounded-3xl p-8 space-y-4 shadow-sm">
        ...
        <div className="space-y-3 text-xs">
          {tor.requirements.map((req, idx) => (
            <div 
              key={idx} 
              className="flex items-start justify-between p-3.5 rounded-xl bg-slate-50 dark:bg-[#121118] border border-slate-100 dark:border-[#2D2938] gap-4"
            >
              ...
              <span 
                className={`px-2.5 py-1 rounded-md text-[10px] font-bold shrink-0 ${
                  req.pass ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'
                }`}
              >
