...
// Reusable Multiselect Component with "Any" logic
interface MultiselectProps {
    ...
    placeholder?: string;
    hasAnyOption?: boolean; // Special flag for "Any" logic
}
...
function Multiselect({ 
    ...
    // Handle toggle with "Any" logic
    const handleToggle = (value: string) => {
        if (hasAnyOption && value === "Any") {
            // If "Any" is selected, clear all other selections
            if (!selected.includes("Any")) {
                // Select only "Any"
                selected.forEach(item => onToggle(item)); // Remove all
                onToggle("Any"); // Add "Any"
            } else {
                // If "Any" is already selected, deselect it
                onToggle("Any");
            }
        } else if (hasAnyOption && selected.includes("Any")) {
            // If "Any" is selected and user selects another option, remove "Any" and add the new option
            onToggle("Any"); // Remove "Any"
            onToggle(value); // Add the new option
        } else {
            // Normal toggle
            onToggle(value);
        }
    ...
    const isAnySelected = hasAnyOption && selected.includes("Any");
    ...
    return (
        <div className="form-control" ref={containerRef}>
                            ...
                            return (
                                <button
                                    ...
                                    className={`
                                        ...
                                        ${isAny && isSelected ? 'bg-primary/5' : ''}
                                        ${isAny ? 'border-t border-base-200/50' : ''}
                                    `}
                                >
                                    ...
                                    <span className={isAny ? 'font-medium text-primary' : ''}>
                                        {option}
                                    </span>
                                </button>
                            );
            ...
            {/* Selected Tags */}
            {selected.length > 0 && !isAnySelected && (
